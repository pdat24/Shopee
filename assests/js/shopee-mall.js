...
async function renderShopeeMallItem() {
    ...
    let totalWidth = 0;
    await fetchData("http://localhost:3000/shopee-mall__product-items").then(
        (items) => {
            ...
            for (const item of items) {
                ...
                fItem.innerHTML = `
                    ...
                    <p class="shopee-mall__product-desc">${
                        item[0].text.length <= 24
                            ? item[0].text
                            : item[0].text.slice(0, 15) + "..."
                    }</p>
                `;
                ...
                if (item.length > 1) {
                    ...
                    sItem.innerHTML = `
                        ...
                        <p class="shopee-mall__product-desc">${
                            item[1].text.length <= 24
                                ? item[1].text
                                : item[1].text.slice(0, 15) + "..."
                        }</p>
                    `;
